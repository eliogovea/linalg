...
public:
  ...
  vector<T, N>(const vector<T_, N>&& other) : values_{std::move(other.values_)} {
    std::cout << "ctor move\n";
  }
  ...
  explicit operator matrix<T, N, 1> () const {
    return matrix<T, N, 1>(values_);
  }
...
vector<util::result_of_add_mul_t<Tm, Tv>, N> 
operator * (const matrix<Tm, N, M>& lhs, const vector<Tv, M>& rhs) {
